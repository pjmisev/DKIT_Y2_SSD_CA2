<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-100 leading-tight">Edit Coach</h2>
                <p class="text-sm text-gray-500 dark:text-gray-400">Update coach details</p>
            </div>
        </div>
    </x-slot>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                <label class="flex items-center gap-3 mt-6 cursor-pointer select-none">
                                    <input type="checkbox" name="status" value="1" {{ old('status', $coach->user->status) ? 'checked' : '' }} class="w-4 h-4 rounded border-gray-300 dark:border-gray-600 text-emerald-600 focus:ring-emerald-500">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <select name="role" id="role"
                                    ...
                                    @foreach ($roles as $role)
                                        <option value="{{ $role }}" {{ old('role', $coach->profileable->role) === $role ? 'selected' : '' }}>{{ $role }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-span-2">
                                ...
                                <textarea name="notes" id="notes" rows="3"
                                    class="input-field">{{ old('notes', $coach->notes) }}</textarea>
                            </div>
                        </div>
                    </div>
