    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">Coach Profile</h2>
            </div>
            @auth
                @if (Auth::user()->isAdmin())
                    <div class="flex items-center gap-3">
                        <a href="{{ route('coaches.edit', $profile) }}" class="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold px-4 py-2 rounded-lg transition-colors">Edit</a>
                        <form method="POST" action="{{ route('coaches.destroy', $profile) }}" class="inline">
                            @csrf @method('DELETE')
                            <button type="submit" onclick="return confirm('Delete this coach?')" class="inline-flex items-center gap-2 bg-red-50 hover:bg-red-100 text-red-600 text-sm font-semibold px-4 py-2 rounded-lg transition-colors">Delete</button>
                        </form>
                    </div>
                @endif
            @endauth
    ...
    <div class="py-10">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-6">
                <div class="md:flex">
                    <div class="md:w-72 h-64 bg-gradient-to-br from-emerald-50 to-emerald-100 flex items-center justify-center">
                        @if ($profile->image)
                            <img src="{{ Storage::url($profile->image) }}" alt="{{ $user->name }}" class="w-full h-full object-cover">
                        @else
                            <div class="w-24 h-24 rounded-full bg-emerald-200 flex items-center justify-center text-emerald-600 font-bold text-3xl">
