    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-100 leading-tight">Coach Profile</h2>
                    <p class="text-sm text-gray-500 dark:text-gray-400">Detailed coach information</p>
                </div>
            </div>
            @auth
                @if (Auth::user()->isAdmin())
                    <div class="flex items-center gap-3">
                        <a href="{{ route('coaches.edit', $profile) }}" class="btn-secondary">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                            Edit
                        </a>
                        <form method="POST" action="{{ route('coaches.destroy', $profile) }}" class="inline">
                            @csrf @method('DELETE')
                            <button type="submit" onclick="return confirm('Delete this coach?')" class="btn-danger">
    ...
    <div class="py-10">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-soft overflow-hidden">
                <div class="md:flex">
                    <div class="md:w-80 min-h-full bg-gradient-to-br from-emerald-500 to-emerald-700 flex items-center justify-center relative overflow-hidden">
                    ...
                    <div class="flex-1 p-8">
                        <div class="flex items-start justify-between mb-6">
                            <div>
                                ...
                                <p class="text-gray-500 dark:text-gray-400 mt-1">{{ $info->specialization ?? 'No specialization set' }}</p>
                            </div>
                        ...
                        <div class="grid grid-cols-2 gap-x-6 gap-y-4 text-sm">
                            ...
                            <div>
                                <span class="text-gray-400 dark:text-gray-500 text-xs">Experience</span>
                                <p class="font-medium text-gray-800 dark:text-gray-200 mt-0.5">{{ $info->years_experience ? $info->years_experience.' years' : '—' }}</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            @if ($info->certifications)
                <div class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-soft p-8">
                    ...
                    <p class="text-gray-600 dark:text-gray-400 text-sm leading-relaxed">{{ $info->certifications }}</p>
                </div>
            @endif
