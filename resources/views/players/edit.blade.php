    ...
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500">Update player details</p>
            </div>
        </div>
    </x-slot>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                <label class="flex items-center gap-3 mt-6 cursor-pointer select-none">
                                    <input type="checkbox" name="status" value="1" {{ old('status', $player->user->status) ? 'checked' : '' }} class="w-4 h-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" name="jersey_number" id="jersey_number" value="{{ old('jersey_number', $player->info->jersey_number) }}" min="0" max="99"
                            ...
                            <div>
                                ...
                                <select name="position" id="position"
                                    ...
                                    @foreach ($positions as $pos)
                                        <option value="{{ $pos }}" {{ old('position', $player->info->position) === $pos ? 'selected' : '' }}>{{ $pos }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select name="dominant_hand" id="dominant_hand" class="input-field">
                                    ...
                                    <option value="left" {{ old('dominant_hand', $player->info->dominant_hand) === 'left' ? 'selected' : '' }}>Left</option>
                                    <option value="right" {{ old('dominant_hand', $player->info->dominant_hand) === 'right' ? 'selected' : '' }}>Right</option>
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <select name="health_status" id="health_status" required
                                    ...
                                    @foreach ($healthStatuses as $hs)
                                        <option value="{{ $hs }}" {{ old('health_status', $player->info->health_status) === $hs ? 'selected' : '' }}>{{ ucfirst($hs) }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <input type="number" name="height_cm" id="height_cm" value="{{ old('height_cm', $player->info->height_cm) }}" min="100" max="250"
                                    class="input-field">
                            </div>

                            <div>
                                <label for="weight_kg" class="form-label">Weight (kg)</label>
                                <input type="number" name="weight_kg" id="weight_kg" value="{{ old('weight_kg', $player->info->weight_kg) }}" min="30" max="200"
                                    class="input-field">
                            </div>

                            <div>
                                <label for="team" class="form-label">Team</label>
                                <input type="text" name="team" id="team" value="{{ old('team', $player->info->team) }}"
                                    class="input-field">
                            </div>

                            <div>
                                <label for="nationality" class="form-label">Nationality</label>
                                <input type="text" name="nationality" id="nationality" value="{{ old('nationality', $player->info->nationality) }}"
                                    class="input-field">
                            </div>

                            <div>
                                <label for="date_of_birth" class="form-label">Date of Birth</label>
                                <input type="date" name="date_of_birth" id="date_of_birth" value="{{ old('date_of_birth', $player->info->date_of_birth?->format('Y-m-d')) }}"
                            ...
                            <div class="col-span-2">
                                ...
                                <textarea name="notes" id="notes" rows="3"
                                    class="input-field">{{ old('notes', $player->info->notes) }}</textarea>
                            </div>
                        </div>
                    </div>
