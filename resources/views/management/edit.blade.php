<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-100 leading-tight">Edit Management</h2>
                <p class="text-sm text-gray-500 dark:text-gray-400">Update management member details</p>
            </div>
        </div>
    </x-slot>
                ...
                <form method="POST" action="{{ route('management.update', $management) }}" enctype="multipart/form-data" class="space-y-8">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                <label class="flex items-center gap-3 mt-6 cursor-pointer select-none">
                                    <input type="checkbox" name="status" value="1" {{ old('status', $management->user->status) ? 'checked' : '' }} class="w-4 h-4 rounded border-gray-300 dark:border-gray-600 text-sky-600 focus:ring-sky-500">
                    ...
                    <div>
                        <div class="flex items-center gap-2 mb-5">
                            ...
                            <h3 class="text-sm font-bold text-gray-800 dark:text-gray-200 uppercase tracking-wider">Management Profile</h3>
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <select name="role" id="role"
                                    ...
                                    @foreach ($roles as $role)
                                        <option value="{{ $role }}" {{ old('role', $management->profileable->role) === $role ? 'selected' : '' }}>{{ $role }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <input type="file" name="image" id="image" accept="image/*"
                                    class="block w-full text-sm text-gray-500 dark:text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-sky-50 dark:file:bg-sky-900/50 file:text-sky-700 dark:file:text-sky-300 hover:file:bg-sky-100 dark:hover:file:bg-sky-800/50">
                            ...
                            <div class="col-span-2">
                                ...
                                <textarea name="notes" id="notes" rows="3"
                                    class="input-field">{{ old('notes', $management->notes) }}</textarea>
                            </div>
                        </div>
                    </div>

                    <div class="flex items-center justify-between pt-4 border-t border-gray-100 dark:border-gray-700">
                        <a href="{{ route('management.index') }}" class="text-sm text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 font-medium transition-colors">Cancel</a>
