<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Add Management Member</h2>
        </div>
    </x-slot>
    ...
    <div class="py-10">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-8">
                <form method="POST" action="{{ route('management.store') }}" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div class="col-span-2">
                                ...
                                <input type="text" name="name" id="name" value="{{ old('name') }}" required
                                    class="block w-full rounded-lg border-gray-300 shadow-sm text-sm focus:border-sky-500 focus:ring-sky-500 @error('name') border-red-400 @enderror">
                            </div>
                            <div class="col-span-2">
                                ...
                                <input type="email" name="email" id="email" value="{{ old('email') }}" required
                                    class="block w-full rounded-lg border-gray-300 shadow-sm text-sm focus:border-sky-500 focus:ring-sky-500 @error('email') border-red-400 @enderror">
                            </div>
                            <div>
                                ...
                                <input type="password" name="password" id="password" required
                                    class="block w-full rounded-lg border-gray-300 shadow-sm text-sm focus:border-sky-500 focus:ring-sky-500 @error('password') border-red-400 @enderror">
                            </div>
                            <div>
                            ...
                            <div>
                                <label class="flex items-center gap-3 mt-6 cursor-pointer">
                                    <input type="checkbox" name="status" value="1" checked class="w-4 h-4 rounded border-gray-300 text-sky-600 focus:ring-sky-500">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div class="col-span-2">
                                ...
                                <textarea name="notes" id="notes" rows="3"
                                    class="block w-full rounded-lg border-gray-300 shadow-sm text-sm focus:border-sky-500 focus:ring-sky-500">{{ old('notes') }}</textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-between pt-2">
                        ...
                        <button type="submit" class="inline-flex items-center gap-2 bg-sky-600 hover:bg-sky-700 text-white text-sm font-semibold px-5 py-2.5 rounded-lg transition-colors">Create Member</button>
                    </div>
                </form>
            </div>
        </div>
