<x-guest-layout>
    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-5">
        ...
        <div class="flex items-center justify-between">
            <label class="flex items-center gap-2 text-sm text-gray-400 cursor-pointer">
                <input type="checkbox" name="remember" class="rounded border-gray-600 bg-white/5 text-hoop-500 focus:ring-hoop-500/30 focus:ring-4">
                Remember me
            </label>
            @if (Route::has('password.request'))
                <a href="{{ route('password.request') }}" class="text-sm text-hoop-400 hover:text-hoop-300 font-medium transition-colors">
                    Forgot password?
                </a>
            @endif
        </div>
    ...
    @if (config('services.google.client_id') || config('services.github.client_id'))
        <div class="divider text-gray-500 mt-6 mb-5">or continue with</div>
    ...
    <p class="mt-6 text-center text-sm text-gray-500">
        ...
        <a href="{{ route('register') }}" class="text-hoop-400 hover:text-hoop-300 font-semibold transition-colors">Sign up</a>
    </p>
</x-guest-layout>
